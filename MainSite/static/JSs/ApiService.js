...
class ApiService {
    ...
    setupTokenRefresh(expiryTimestamp) {
        ...
        const timeUntilRefresh = (expiryTimestamp * 1000) - Date.now() - refreshBuffer;
    ...
    async getAllTasks(filters = {}) {
        ...
        return this.request(`/tasks${queryParams ? '?' + queryParams : ''}`, { method: 'GET' });
    }
    ...
    async getTasksByTeacher(teacherId, filters = {}) {
        const queryParams = new URLSearchParams({ teacherId, ...filters }).toString();
    ...
    async getUsers(filters = {}) {
        const queryParams = new URLSearchParams(filters).toString();
        return this.request(`/users${queryParams ? '?' + queryParams : ''}`, { method: 'GET' });
    }
